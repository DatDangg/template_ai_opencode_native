...
function detectDb(sourceRoots) {
  // Root + mọi package con (monorepo) — schema/config ORM thường nằm trong từng package.
  const dirs = [ROOT, ...sourceRoots.map((r) => join(ROOT, r))];
  ...
  for (const dir of dirs) {
    for (const f of ["drizzle.config.ts", "drizzle.config.js", "drizzle.config.mjs"]) {
      if (existsSync(join(dir, f))) return { db_tool: "drizzle", migration_required: true };
    }
  }
...
function main() {
  ...
  const { db_tool, migration_required } = detectDb(source_roots);
